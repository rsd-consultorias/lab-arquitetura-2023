...
describe('REQ02 - Análise de Score', () => {
    ...
    it('Verificar score elegível', async () => {
        let correntistaScoreElegivel: { dados: Correntista, mensagem?: string } = await correntistaService.cadastrarCorrentistaAsync(proponentes.comum)
    ...
    it('Verificar score baixo', async () => {
        let correntistaScoreBaixo: { dados: Correntista, mensagem?: string } = await correntistaService.cadastrarCorrentistaAsync(proponentes.scoreBaixo)
